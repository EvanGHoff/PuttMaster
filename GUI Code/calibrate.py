...
def display_markers():
    ...
    markers = []
    for i in range(4):
        marker = cv2.imread(f'markers/marker_{i}.png')
...
def my_warp(image):
    ...
    Aruco_cam = pickle.load(open('matrixes/srcPts.p','rb'))

    matrix1 = cv2.getPerspectiveTransform(Aruco_cam, Aruco_proj)

    green_cam = pickle.load(open('matrixes/dstPts.p','rb'))
...
def main(cap):
    ...
    cv2.destroyAllWindows()
    pickle.dump(src_points, open('matrixes/srcPts.p','wb'))
    ...
    pickle.dump(dst_points, open('matrixes/dstPts.p','wb'))
    #input()  pause to find points

    matrix = cv2.getPerspectiveTransform(src_points, dst_points)
    pickle.dump(matrix, open('matrixes/matrix.p','wb'))
